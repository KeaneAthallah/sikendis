<?php 
/**
 * File			: unit_script.php
 * Description  : Script untuk halaman Master > OPD > Unit
...
<script>
    ...
    $(function() {
        _datatable = $('#dt-<?= $mod ?>').DataTable({
            ...
            ajax: {
                url: _HOST+'master/opd/unit/get-datatable',
                type: 'POST',
                data: function (d) {
            ...
            columns: [
                {data:"kd_unit", sortable: true, width: "20%", class: "text-sm"},
                {data:"nm_unit", sortable: true, width: "30%", class: "text-sm"},
                {data:"nm_opd", sortable: true, width: "30%", class: "text-sm"},
            ...
            drawCallback: function(settings) { 
                ...
                $(this).on('click', '#btn-view', function(e) {
                    ...
                    openModal(_HOST+'master/opd/unit/get-form?sts=view&id='+_id, 'modal-lg', 'View Unit', event);
                });
                $(this).on('click', '#btn-edit', function (e) {
                    let _id = $(this).data("id");
                    openModal(_HOST+'master/opd/unit/get-form?sts=edit&id='+_id, 'modal-lg', 'Edit Unit', event);
                });
                $(this).on('click', '#btn-delete', function (e) {
                    let _id = $(this).data("id");
                    let _desc = $(this).data("desc");
                    let _msg = 'Anda akan menghapus data Unit: ' + _desc + '.';
                    deleteData(_HOST+'master/opd/unit/delete', _id, _msg, event);
                });
            },
        ...
        $('#list-unit #btn-add').on('click', function(e) {
            openModal(_HOST+'master/opd/unit/get-form?sts=add', 'modal-lg', 'Input Unit', e);
        });
        ...
        $(document).on('submit', '#form-unit', function(e) {
            ...
            let _url = $(this).attr("action");
      		...
      		$.ajax({
      			...
      			processData: false,
      			beforeSend: function() {
                    $('#form-unit #btn-save').attr('disabled', 'disabled');
                    $('#form-unit #btn-save').html('<span class="spinner-border spinner-border-sm me-1"></span> Proses...');
      			},
      			complete: function() {
                    $('#form-unit #btn-save').removeAttr('disabled');
                    $('#form-unit #btn-save').html('<span class="tf-icons bx bx-save me-1"></span> Simpan');
      			},
      			success: function(result) {
      				...
      				if (_res.rc == '1') {
      					swalMsg('Sukses', _res.rd, 'success', function() {
                            $(document).find('#form-unit #btn-close').trigger('click');
      					});
      				} else {
      					let _error = '';
