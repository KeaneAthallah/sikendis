...
/**
 * File			: opd_script.php
 * Description  : Script untuk halaman Master > OPD > OPD
...
<script>
    ...
    $(function() {
        _datatable = $('#dt-<?= $mod ?>').DataTable({
            ...
            ajax: {
                url: _HOST+'master/opd/opd/get-datatable',
                type: 'POST',
                data: function (d) {
            ...
            columns: [
                {data:"kd_opd", sortable: true, width: "20%", class: "text-sm"},
                {data:"nm_opd", sortable: true, width: "60%", class: "text-sm"},
            ...
            drawCallback: function(settings) { 
                ...
                $(this).on('click', '#btn-view', function(e) {
                    ...
                    openModal(_HOST+'master/opd/opd/get-form?sts=view&id='+_id, 'modal-lg', 'View OPD', event);
                });
                $(this).on('click', '#btn-edit', function (e) {
                    let _id = $(this).data("id");
                    openModal(_HOST+'master/opd/opd/get-form?sts=edit&id='+_id, 'modal-lg', 'Edit OPD', event);
                });
                $(this).on('click', '#btn-delete', function (e) {
                    let _id = $(this).data("id");
                    let _desc = $(this).data("desc");
                    let _msg = 'Anda akan menghapus data OPD: ' + _desc + '.';
                    deleteData(_HOST+'master/opd/opd/delete', _id, _msg, event);
                });
            },
        ...
        $('#list-opd #btn-add').on('click', function(e) {
            openModal(_HOST+'master/opd/opd/get-form?sts=add', 'modal-lg', 'Input OPD', e);
        });

        $(document).on('submit', '#form-opd', function(e) {
      		e.preventDefault(); 
            let _url = $(this).attr("action");
      		...
      		$.ajax({
      			...
      			processData: false,
      			beforeSend: function() {
                    $('#form-opd #btn-save').attr('disabled', 'disabled');
                    $('#form-opd #btn-save').html('<span class="spinner-border spinner-border-sm me-1"></span> Proses...');
      			},
      			complete: function() {
                    $('#form-opd #btn-save').removeAttr('disabled');
                    $('#form-opd #btn-save').html('<span class="tf-icons bx bx-save me-1"></span> Simpan');
      			},
      			success: function(result) {
      				...
      				if (_res.rc == '1') {
      					swalMsg('Sukses', _res.rd, 'success', function() {
                            $(document).find('#form-opd #btn-close').trigger('click');
      					});
      				} else {
      					let _error = '';
