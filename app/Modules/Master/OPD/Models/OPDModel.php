<?php namespace App\Modules\Master\OPD\Models;
/**
 * File			: OPDModel.php
 * Description  : Model untuk halaman Master > OPD > OPD
...
class OPDModel extends Model {
    ...
    protected $allowedFields = ['kd_opd', 'nm_opd'];
	...
	public function getDatatable($par=null) {
		...
		$datatables->select("a.id, a.kd_opd, a.nm_opd", FALSE)
					->from('opd a')
					->addColumn('actions', 
								'<td>
	...
	public function isUsed($id) {
		$jml = $this->builder()->selectCount('id')
			->join('unit u', 'opd.id = u.opd_id', 'inner')
			->where('u.opd_id', $id)
			->countAllResults(); 
