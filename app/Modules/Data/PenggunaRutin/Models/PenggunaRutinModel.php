<?php namespace App\Modules\Data\PenggunaRutin\Models;
/**
 * File			: PenggunaRutinModel.php
 * Description  : Model untuk halaman Data > Penggunaan > Pengguna Rutin
...
class PenggunaRutinModel extends Model {
    protected $table = 'pengguna_rutin';
	protected $primaryKey = 'id';
	protected $useAutoIncrement = true;
	# soft delete artinya bukan hapus data permanen, tapi saat call method delete() hanya ngisi deleted_at saja
	protected $useSoftDeletes = false;
    protected $allowedFields = ['kendaraan_id', 'pengguna_id', 'no_bap', 'tgl_bap', 'file_bap'];
	...
	public function getDatatable($par=null) {
		...
		$datatables->select("a.id, b.no_polisi, CONCAT(c.nama_lengkap, ' (', c.nip, ')') AS pengguna, a.no_bap, a.file_bap", FALSE)
					->from('pengguna_rutin a')
					->join('kendaraan b', 'a.kendaraan_id = b.id', 'inner')
					->join('pengguna c', 'a.pengguna_id = c.id', 'inner');

		if (session()->get('level_id') == 2) # operator
			$datatables->where('c.unit_id', session()->get('unit_id'));
		...
		$datatables->addColumn('actions',
								'<td>
									<div class="d-flex align-items-center">
										...
										<button type="button" id="btn-delete" class="btn btn-sm btn-icon btn-danger mb-0 py-1 px-2" data-mod="$1" data-id="$2" data-desc="$3 No. BAP $4" title="Hapus">
										...
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-end" style="">
												<a href="../../$5" class="dropdown-item" target="_blank">Lihat File BAP</a>
											</div>
										</div>
									</div>
								</td>',
								'pengguna-rutin, id, pengguna, no_bap, file_bap'); # ../../ itu pengganti site_url() karena ga bisa ditambahkan di situ
